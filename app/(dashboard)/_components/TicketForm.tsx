...
const TicketForm = ({ onTicketCreated }) => {
  ...
  return (
    <div className="max-w-3xl mx-auto animate-slide-up-fade">
      <div className="relative bg-app-panel backdrop-blur-sm rounded-2xl border border-app overflow-hidden shadow-sm">
        ...
        <div className="border-b border-app-subtle px-5 sm:px-7 py-5 sm:py-6">
          <div className="flex items-start gap-3.5">
            <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-app-primary shadow-lg">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
              </svg>
            </div>
            <div className="min-w-0">
              <h2 className="text-xl sm:text-2xl font-bold text-app tracking-tight">
        ...
        <form onSubmit={handleSubmit} className="p-5 sm:p-7 space-y-6 sm:space-y-7">
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between gap-3">
              <label htmlFor="title" className="block text-sm font-semibold text-app">
                Issue title <span className="text-app-primary">*</span>
              </label>
              <span className={`text-xs tabular-nums ${titleNearLimit ? 'text-amber-500' : 'text-app-muted'}`}>
            ...
            <input
              ...
              className="app-field w-full px-4 py-3 sm:py-3.5 border rounded-xl shadow-sm transition-all duration-200 disabled:opacity-60"
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between gap-3">
              <label htmlFor="description" className="block text-sm font-semibold text-app">
                Description <span className="text-app-primary">*</span>
              </label>
              <span className={`text-xs tabular-nums ${descNearLimit ? 'text-amber-500' : 'text-app-muted'}`}>
            ...
            <textarea
              ...
              maxLength={1000}
              disabled={loading || success}
              className="app-field w-full px-4 py-3 sm:py-3.5 border rounded-xl shadow-sm transition-all duration-200 resize-y min-h-[120px] disabled:opacity-60"
          ...
          <div className="space-y-3">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1">
              <label className="block text-sm font-semibold text-app">
                Screenshots <span className="font-normal text-app-muted">(optional)</span>
              </label>
            ...
            {attachmentFiles.length < MAX_ATTACHMENTS && (
              <button
                ...
                className={`w-full rounded-xl border-2 border-dashed transition-all duration-200 p-4 sm:p-5 group disabled:opacity-60 disabled:cursor-not-allowed ${
                  isDragging
                    ? 'border-app-primary bg-app-primary-soft scale-[1.01]'
                    : 'border-app bg-app-surface-2/40 hover:bg-app-primary-soft hover:border-app-primary'
                }`}
              >
                <div className="flex flex-col sm:flex-row items-center justify-center gap-3 text-center sm:text-left">
                  <div className="p-2.5 rounded-xl bg-app-primary-soft border border-app-primary text-app-primary group-hover:scale-105 transition-transform">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-app-muted mt-0.5">PNG, JPG, WEBP, GIF</p>
                  </div>
                </div>
              </button>
            )}
            ...
            {attachmentPreviews.length > 0 && (
              <div className="grid grid-cols-3 gap-2.5 sm:gap-3">
                {attachmentPreviews.map((preview, index) => (
                  <div
                    key={`${preview}-${index}`}
                    className="relative group rounded-xl overflow-hidden border border-app bg-app-surface-2 aspect-square"
                  >
                    <img
                      src={preview}
          ...
          <div className="space-y-3">
            <label className="block text-sm font-semibold text-app">
              Priority <span className="text-app-primary">*</span>
            </label>
                ...
                return (
                  <label
                    ...
                    className={`relative flex flex-col items-start gap-1 rounded-xl border px-3 py-3 cursor-pointer transition-all duration-200 ${
                      selected
                        ? priority.active
                        : 'bg-app-surface-2/50 border-app text-app-soft hover:border-app-primary hover:bg-app-surface-2'
                  ...
                  >
                    ...
                    <span className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-semibold text-app">{priority.label}</span>
                    </span>
                    <span className="text-[11px] text-app-muted leading-snug">{priority.hint}</span>
                  </label>
                );
              })}
            </div>
          ...
          <div className="space-y-3">
            <label className="block text-sm font-semibold text-app">
              Category <span className="text-app-primary">*</span>
            </label>
                ...
                return (
                  <label
                    ...
                    className={`flex items-center gap-3 rounded-xl border px-3 py-3 cursor-pointer transition-all duration-200 ${
                      selected
                        ? 'bg-app-primary-soft border-app-primary text-app-primary ring-1 ring-app-primary/25'
                        : 'bg-app-surface-2/50 border-app text-app-soft hover:border-app-primary hover:bg-app-surface-2'
                    } ${loading || success ? 'opacity-60 pointer-events-none' : ''}`}
                  >
                    <input
                    ...
                    <span
                      className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-lg ${
                        selected ? 'bg-app-primary-soft text-app-primary' : 'bg-app-surface-3 text-app-muted'
                      }`}
                    >
                      <CategoryIcon type={category.value} />
                    </span>
                    <span className="min-w-0">
                      <span className="block text-sm font-semibold text-app">{category.label}</span>
                      <span className="block text-[11px] text-app-muted truncate">{category.description}</span>
                    </span>
                  </label>
                );
              })}
          ...
          <div className="flex flex-wrap items-center gap-2 text-xs">
            <span className="text-app-muted">Submitting as</span>
            <span className="inline-flex items-center gap-1.5 rounded-full bg-app-surface-2 border border-app px-2.5 py-1 text-app-soft">
              <span className={`h-1.5 w-1.5 rounded-full ${selectedPriority?.dot}`} />
              {selectedPriority?.label}
            </span>
            <span className="inline-flex items-center gap-1.5 rounded-full bg-app-surface-2 border border-app px-2.5 py-1 text-app-soft">
              <CategoryIcon type={selectedCategory?.value} className="w-3.5 h-3.5 text-app-primary" />
          ...
          <div className="pt-2 space-y-2">
            ...
            <button
              ...
              className="w-full inline-flex items-center justify-center gap-2 rounded-xl bg-app-primary disabled:bg-app-surface-3 disabled:text-app-muted disabled:cursor-not-allowed text-sm sm:text-base font-semibold px-5 py-3.5 transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-app-primary/50"
            >
              {loading ? (
                <>
