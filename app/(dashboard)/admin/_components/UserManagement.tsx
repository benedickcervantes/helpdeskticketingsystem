...
const UserManagement = () => {
  ...
  const handleDeleteUser = async (userId) => {
    if (window.confirm('Are you sure you want to delete this user? Users with existing ticket activity will be deactivated instead of permanently deleted.')) {
      try {
        setError('');
        setNotice('');
        const result = await api.delete(`/api/v1/users/admin/${userId}`);
        setNotice(result?.message || 'User deleted successfully.');
        loadUsers();
      } catch (error) {
  ...
  return (
    <div className="bg-gray-800/50 backdrop-blur-sm rounded-xl border border-gray-700 p-3 sm:p-4 md:p-6 user-management-robust-fix">
      ...
      {notice && (
        <div className="bg-emerald-900/30 border border-emerald-700 text-emerald-300 px-3 sm:px-4 py-2 sm:py-3 rounded-xl text-xs sm:text-sm mb-4">
          {notice}
        </div>
      )}
